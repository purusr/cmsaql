...
function listModulePage(type, props = {}) {
    return nsModules.filter((m) => m.type === type).map((m, index) => {
        ...
        return require('react').createElement(Comp, {key: index + m.code, gNext: { Head, Link }, ...props})
    });
}
...
export {
    slugify, countProductInCartByCategory, countProductInCartByProduct, listModulePage, getModulesHookFunctionsByType
};
